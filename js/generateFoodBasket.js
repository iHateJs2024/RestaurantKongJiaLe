...
basketItems = JSON.parse(localStorage.getItem('basketItems')) || [];
  ...
  function addObject(selectedItem, itemPrice, takeAwayPackaging, quantity, notes, img) {
    basketItems.push({name: selectedItem, price: itemPrice, takeAwayPackaging: takeAwayPackaging, quantity: quantity, notes: notes, img: img});
  ...
  function generateBasketItems() {
    ...
    let CheckoutSummaryDetails = document.getElementById('checkout-summary-details');

    if (basketItems.length !== 0) {
      document.getElementById('basket-items').innerHTML = basketItems.map((basketItem)=> {
        let { id, name, price, takeAwayPackaging, quantity, notes, img } = basketItem;
            ...
            <div class="first-vertical-layout">
              ...
              ">
                <div id="basket-item-name-and-price" class="basket-item-name-and-price">
                  <p>${name}</p>
                  <strong class="item-price">RM${price * quantity}.00</strong>
                </div>
                <div class="basket-takeaway-packaging-price">
                  <p>Pembungkusan Bawa pulang</p>
                  <strong>RM${takeAwayPackaging}.00</strong>
                </div>
      ...
      document.querySelectorAll('.first-vertical-layout').forEach((itemElement, index) => {
        // Add event listener for increment button
        itemElement.querySelector('.plus-button-basket').addEventListener('click', () => {
          ...
          CalculateTotalPriceBasket();
          updateTotalQuantity();
          updateItemPrice(itemElement, basketItems[index].quantity, basketItems[index]);
        });
    
        // Add event listener for decrement button
        itemElement.querySelector('.minus-button-basket').addEventListener('click', () => {
  ...
  let incrementQuantityBasket = (quantityID) => {
    ...
    let searchQuantity = basketItems.find((basketItem) => basketItem.id === selectedQuantity);
  ...
  function totalPriceModalBasketItem(quantityID, itemPrice, takeAwayPackaging, totalPriceID) {
    
    let quantity = document.getElementById(quantityID).innerHTML;
    let totalPrice = (quantity * itemPrice) + takeAwayPackaging;

    document.getElementById(totalPriceID).innerHTML = `RM${totalPrice}.00`;
  };
  ...
  function updateItemPrice(itemElement, quantity, selectedItemIndex) {
    ...
    itemElement.querySelector('.item-price').textContent = `RM${selectedItemIndex.price * quantity}.00`;
  }

  /// Update Subtotal function
  function updateSubtotal() {
    // console.log(basketItems);
    const subTotal = basketItems.reduce((sum, item) => sum + item.price * item.quantity + item.takeAwayPackaging, 0);
    // console.log('Subtotal:', subTotal);

    document.getElementById('subtotal-basket').innerHTML = `RM${subTotal}.00`;
  };

  /// Update Tax function
  function updateTax() {
    let subTotal = document.getElementById('subtotal-basket').textContent;
    let tax = parseFloat(subTotal.replace("RM", ""));
    tax = tax * 6 / 100;
    tax = tax.toFixed(2);
  ...
  function checkRoundingBasket() {
    ...
    let basketRoundingAndPrice = document.getElementById('rounding-and-price-basket');
  ...
  //? SUBTOTAL
  let subtotal = parseFloat(document.getElementById('subtotal-basket').textContent.replace("RM", ""));
  // console.log(subtotal);
  
  //? Total
  let tax = parseFloat(document.getElementById('tax-basket').textContent.replace("RM", ""));
  // console.log(tax);

  //? Rounding
  let rounding = parseFloat(document.getElementById('rounding-price').textContent.replace("RM", ""));
  // console.log(rounding);
  ...
  function updateTotalQuantity() {
    // console.log(basketItems);
    const totalQuantity = basketItems.reduce((totalQuantity, item) => totalQuantity + item.quantity, 0);
    // console.log(totalQuantity);
    
    if (totalQuantity === 1) {
      document.getElementById('checkout-item').innerHTML = `${totalQuantity} ITEM`;
    } else {
      document.getElementById('checkout-item').innerHTML = `${totalQuantity} ITEMS`;
    }
  };
